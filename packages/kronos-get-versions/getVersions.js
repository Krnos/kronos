...
module.exports = async function getVersions (local) {
  if (sessionCached) {
  ...
  if (process.env.CONTROLLA_CLI_TEST || process.env.CONTROLLA_CLI_DEBUG) {
    return (sessionCached = {
      current: local,
      latest: local
    })
...
async function getAndCacheLatestVersion (cached) {
  ...
  const res = await getPackageVersion('controlla-cli-version-marker', 'latest')
