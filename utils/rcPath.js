...
const xdgConfigPath = file => {
  ...
  if (xdgConfigHome) {
    const rcDir = path.join(xdgConfigHome, 'controlla')
...
// we introduced a change storing .controllarc in AppData, but the benefit isn't
...
const migrateWindowsConfigPath = file => {
  ...
  if (appData) {
    const rcDir = path.join(appData, 'controlla')
